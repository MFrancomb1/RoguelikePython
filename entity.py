...
class Entity:
    ...
    def __init__(
        ...
        if parent:
            #if parent isnt provided now then it will be later.
            self.gamemap = parent
    ...
    def place(self, x: int, y: int, gamemap: Optional[GameMap] = None) -> None:
        ...
        self.y = y
        if gamemap:
            if hasattr(self, "parent"): #possibly uninitialaized
    ...
    def distance(self, x: int, y: int) -> float:
        """Return the distance between the current entity and the give (x,y) coordinate."""
...
class Actor(Entity):
    ...
    def is_alive(self) -> bool:
        """Returns true if the actor can perform actions"""
        return bool(self.ai)
